# NYC Taxi (TLC 2017) Analysis Series

This directory contains a multi-stage analytical investigation into the 2017 New York City Taxi & Limousine Commission (TLC) dataset, moving from raw data to predictive modeling.



## Project Folders

* **[01_Exploratory_Data_Analysis](./01_Exploratory_Data_Analysis/README.md):** Initial phase focused on data cleaning, outlier detection, and visualization of trip trends using Python and Tableau.
* **[02_Statistical_Analysis](./02_Statistical_Analysis/README.md):** Application of hypothesis testing and inferential statistics to verify significant differences in vendor performance and fare distributions.
* **[03_Linear_Regression](./03_Linear_Regression/README.md):** Development of a baseline regression model to quantify the predictive relationship between trip distance and total fare amounts.
<!-- * **[03_Machine_Learning](./04_Machine_Learning/README.md):** Understanding and fundamentals of using machine learning to predicit future outcomes and make forecast observations. -->
* **[data](./data/2017_Yellow_Taxi_Data.csv):** Centralized repository for the raw and processed CSV files utilized across the various stages of the project.



---

[← Back to Main Portfolio](../README.md)
